...
def send_invitation_email(group, email, request):
    subject = f"You're invited to join the group '{group.name}' on ChipIn!"
    invite_link = request.build_absolute_uri(
        reverse('chipin:accept_invite', args=[group.id])
    )
    message = f"Hi there,\n\nYou have been invited to join the group '{group.name}' on ChipIn.\n"
    message += f"Click the link below to join:\n{invite_link}\n\nBest regards,\nThe ChipIn Team"
